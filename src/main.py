    ...
    def RACE():
        ...
        while lap_number < track.lap:
            ...
            for driver in f1tv:
                ...
                nonindividual = (track.time + (loss_of_the_car)*0.5 + total_pit + (round(driver[28])*10) - (driver[28]*10) + ((track.time*3)/90) -(2.5))*1.01
                individual = (-(uniform(0.05,0.35)*form -(track.time/60))*0.25 + uniform(-(racecraft**3/1000000), ((100-racecraft)**3/50000)) + uniform(0, (fail_odd/5)) + uniform(-(form**2/20), ((3-form)**2/20)))*0.1
            ...
            while igo<len(mx):
                try:
                    ...
                    if defender_defence + randint(-50,450) > attacker_attack:
                        ...
                        if (attacker_agression > defender_agression*(f1.stamina+1)) and (attacker_name and defender_name not in dnf0):

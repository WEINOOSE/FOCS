    ...
    def RACE():
        ...
        while lap_number < track.lap:
            ...
            for driver in f1tv:
                ...
                nonindividual = track.time + loss_of_the_car*2 + total_pit + (round(driver[28])*10) - (driver[28]*10) + ((track.time*3)/90) - uniform(0.05,0.35)*form -(track.time/60)
            ...
            while igo<len(mx):
                try:
                    ...
                    if defender_defence + randint(-50,450) > attacker_attack:
                        s_dict[attacker_name] += losing_time + 7.5
                        s_dict[defender_name] += losing_time - 2.5
                    else:
                        if (attacker_agression > defender_agression*(f1.stamina+0.75)) and (attacker_name and defender_name not in dnf0):
                            ...
                            s_dict[attacker_name] += losing_time + 4.5
                            s_dict[defender_name] += losing_time + 5.5     
                    igo += 2
                except:
                    igo += 2

...
def START(tracknamespace):
    ...
    while i<len(mx):
        ...
        try:
            ...
            attacker = (((((att2)/100) + (agg2)*7)/5) + (randint(0,10)/(f1.collision+1)))
    ...
    def RACE():
        ...
        while lap_number < track.lap:
            ...
            for driver in f1tv:
                ...
                lape_of_a_moment = uniform((-driver[11]/55),(driver[11]/195)) # Between -0.5 and +0.5
                nonindividual = track.time + (loss_of_the_car*2) + total_pit + (round(driver[28])*10) - (driver[28]*10) + ((track.time*2.5/110)) + lape_of_a_moment
                individual = uniform(-(racecraft**3/1000000), ((100-racecraft)**3/50000)) + uniform(0, (fail_odd/5)) + uniform(-(form**2/20), ((3-form)**2/20))
                ...
                if (sc_alert == 1 or crash_alert == 1) and (tires >= 7):
                    if (randint(0,100) < 35):
                        laptime += 8
                        tires = 0
                        tyre_selection = f1.h
                        ...
                        pass
                elif (sc_alert == 1 or crash_alert == 1) and (tires >= 10):
                        laptime += 8
                        tires = 0
                        tyre_selection = f1.h
                        lastik_dict[name] = 100
            ...
            while igo<len(mx):
                try:
                    ...
                    if defender_defence + randint(-25,75) > attacker_attack:
                        s_dict[attacker_name] += losing_time + 2.5
                        s_dict[defender_name] += losing_time - 1.5
                    else:
                        if (attacker_agression > defender_agression*(f1.stamina+0.25)) and (attacker_name and defender_name not in dnf0):
                            if (attacker_agression > defender_agression*f1.stamina*f1.stamina):
                                ...
                                dnf0.append(defender_name)
                                s_dict[attacker_name] += losing_time*3
                                crash_alert = 1
                        else:
                            s_dict[attacker_name] += losing_time + 1.5
                            s_dict[defender_name] += losing_time + 3.0     
                    igo += 2
                except:
                    igo += 2
